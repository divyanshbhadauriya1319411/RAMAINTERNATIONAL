...
const COLUMNS = [
  { key: "APPLIED", labelKey: "colApplied", color: "border-blue-500 bg-blue-500/10 text-blue-400" },
  { key: "SHORTLISTED", labelKey: "colShortlisted", color: "border-yellow-500 bg-yellow-500/10 text-yellow-400" },
  { key: "INTERVIEW_SCHEDULED", labelKey: "colInterviewing", color: "border-indigo-500 bg-indigo-500/10 text-indigo-400" },
  { key: "SELECTED", labelKey: "colSelected", color: "border-green-500 bg-green-500/10 text-green-400" },
  { key: "VISA_STAGE", labelKey: "colVisa", color: "border-gold-500 bg-gold-500/10 text-gold-400" },
  { key: "MOBILIZED", labelKey: "colMobilized", color: "border-purple-500 bg-purple-500/10 text-purple-400" },
];
...
export default function PipelineBoard({ applications, onRefresh }: PipelineBoardProps) {
  ...
  const t = useTranslations("pipelineBoard");
          ...
          return (
            <div key={col.key} className="bg-navy-900 border border-gold-500/10 rounded-xl p-4 min-w-[220px] flex flex-col">
              <div className="flex justify-between items-center pb-3 mb-3 border-b border-gold-500/5">
                <h3 className="font-semibold text-xs text-gold-500 uppercase tracking-widest">{t(col.labelKey as any)}</h3>
        ...
        <div className="fixed inset-0 z-50 bg-black/60 backdrop-blur-sm flex items-center justify-center p-4">
          <div className="bg-navy-900 border border-gold-500/30 rounded-xl max-w-lg w-full text-white shadow-2xl p-6 sm:p-8 animate-in zoom-in-95 duration-200">
            <div className="flex justify-between items-start border-b border-gold-500/15 pb-4 mb-4">
              <div>
                <h3 className="font-serif text-lg font-bold text-gold-500">{t("modalTitle")}</h3>
                <p className="text-xs text-gray-400">{t("modalDesc")}</p>
              </div>
              <button
            ...
            <div className="bg-navy-850 rounded-lg p-4 mb-6 border border-gold-500/10 text-xs grid grid-cols-2 gap-3">
              <div>
                <span className="text-gray-400 block font-semibold mb-0.5">{t("labelCandidate")}</span>
                <span className="text-gray-100 font-semibold">{selectedApp.candidate.fullName}</span>
              </div>
              <div>
                <span className="text-gray-400 block font-semibold mb-0.5">{t("labelVacancy")}</span>
                <span className="text-gray-100 font-semibold">{selectedApp.job.title}</span>
              </div>
              <div>
                <span className="text-gray-400 block font-semibold mb-0.5">{t("labelExperience")}</span>
                <span className="text-gray-205">{selectedApp.candidate.experienceYears || 0} {t("years")}</span>
              </div>
              <div>
                <span className="text-gray-400 block font-semibold mb-0.5">{t("labelResume")}</span>
                {selectedApp.candidate.resumeUrl ? (
                  ...
                  <span className="text-gray-500">{t("notUploaded")}</span>
                )}
              </div>
            </div>
            ...
            <div className="space-y-4 text-xs">
              <div>
                <label className="block text-gray-400 font-semibold mb-1">{t("stage")}</label>
                <select
                  value={editStatus}
                ...
                >
                  {COLUMNS.map((col) => (
                    <option key={col.key} value={col.key} className="text-navy-900">
                      {t(col.labelKey as any)}
                    </option>
                  ))}
                  <option value="REJECTED" className="text-navy-900">{t("rejected")}</option>
                </select>
              </div>

              {editStatus === "INTERVIEW_SCHEDULED" && (
                <div>
                  <label className="block text-gray-400 font-semibold mb-1">{t("interviewTime")}</label>
                  <input
                    type="datetime-local"
              ...
              {editStatus === "VISA_STAGE" && (
                <div>
                  <label className="block text-gray-400 font-semibold mb-1">{t("visaSubStage")}</label>
                  <select
                    value={editVisaStatus}
                    onChange={(e) => setEditVisaStatus(e.target.value)}
                    className="w-full bg-navy-800 border border-gold-500/20 text-gray-100 rounded-lg p-2.5 outline-none focus:border-gold-500 transition-colors"
                  >
                    <option value="DOCUMENT_VERIFICATION" className="text-navy-900">Document Verification</option>
                    <option value="EMBASSY_SUBMISSION" className="text-navy-900">Embassy Submission</option>
                    <option value="VISA_STAMPED" className="text-navy-900">Visa Stamped</option>
                    <option value="FLIGHT_BOOKED" className="text-navy-900">Flight Booked</option>
                    <option value="DEPLOYED" className="text-navy-900">Deployed</option>
                  </select>
                </div>
              )}

              <div>
                <label className="block text-gray-400 font-semibold mb-1">{t("notesLabel")}</label>
                <textarea
                  rows={3}
                  value={editNotes}
              ...
              <button
                ...
                className="w-full bg-gradient-to-r from-gold-500 to-gold-600 hover:from-gold-400 hover:to-gold-500 text-navy-950 font-bold py-3 rounded-lg flex items-center justify-center space-x-2 text-sm transition-all shadow-lg hover:shadow-gold-500/10 cursor-pointer border-none"
              >
                {updating ? (
