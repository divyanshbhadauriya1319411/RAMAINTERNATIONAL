...
export default function SuperAdminPanel() {
  ...
  const fetchAdminData = async () => {
    ...
    try {
      ...
      setCandidates([
        { id: "cand-1", fullName: "Rahul Sharma", phone: "+91 9876543210", passportNumber: "Z1234567", resumeUrl: "/uploads/resumes/Verified_Rahul_CV.pdf", skills: "Electrician, maintenance", experienceYears: 5, education: "ITI Diploma", location: "Delhi, India" },
        { id: "cand-2", fullName: "Amit Kumar", phone: "+91 9822453671", passportNumber: "Y8765432", resumeUrl: null, skills: "TIG Welder, structural piping", experienceYears: 8, education: "Vocational Certificate", location: "Mumbai, India" },
        { id: "cand-3", fullName: "Priya Nair", phone: "+91 9711223344", passportNumber: "X9876543", resumeUrl: "/uploads/resumes/Priya_Nursing_Resume.pdf", skills: "Staff Nurse, Critical Care ICU", experienceYears: 3, education: "B.Sc Nursing", location: "Kochi, India" },
      ]);
  ...
  const getStatusBadge = (status: string) => {
    switch (status) {
      ...
      case "SHORTLISTED":
        return "bg-yellow-50 text-yellow-800 border-yellow-250";
      ...
      <div className="bg-[#020C1B] text-white py-12 border-b border-gold-500/20">
        <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 flex flex-col sm:flex-row justify-between items-start sm:items-center gap-4">
          <div className="space-y-1">
            <h1 className="font-serif text-2xl font-bold text-gold-500">Super Admin Console</h1>
            <p className="text-xs text-gray-300 font-light">Liaison and global operations oversight control board.</p>
          </div>

          <div className="bg-navy-900/60 border border-gold-500/10 rounded-xl p-3.5 flex items-center space-x-3 text-xs font-bold">
            <Lock className="h-4.5 w-4.5 text-gold-500" />
            <div>
              <p className="text-gray-200">Deepak Chauhan</p>
            ...
            >
                ...
                <div className="space-y-6">
                  <div className="bg-white border border-gray-200 rounded-xl p-6 shadow-sm flex flex-col sm:flex-row justify-between items-start sm:items-center gap-4">
                    <div>
                      ...
                      <p className="text-[11px] text-gray-500 font-light font-sans font-medium">RAMA INTERNATIONAL gross statistics panel.</p>
                    </div>
                    <button
                      onClick={() => setRefreshTrigger((prev) => prev + 1)}
                      className="border border-gold-500/35 hover:bg-gold-500 hover:text-navy-900 text-gold-600 text-xs font-bold uppercase tracking-wider px-4 py-2.5 rounded-lg flex items-center space-x-1.5 transition-colors cursor-pointer"
                    >
                  ...
                  <div className="grid grid-cols-2 lg:grid-cols-4 gap-6 text-xs font-semibold">
                    <div className="bg-white border border-gray-200 rounded-xl p-5 shadow-sm space-y-1">
                      <span className="text-[10px] text-gray-400 font-bold uppercase tracking-wider block">Job Campaigns</span>
                      <span className="text-2xl font-serif font-extrabold text-navy-900">{jobs.length}</span>
                    </div>
                    <div className="bg-white border border-gray-200 rounded-xl p-5 shadow-sm space-y-1">
                      <span className="text-[10px] text-gray-400 font-bold uppercase tracking-wider block">Active Visas</span>
                      <span className="text-2xl font-serif font-extrabold text-navy-900">{applications.filter((a) => a.status === "VISA_STAGE").length}</span>
                    </div>
                    <div className="bg-white border border-gray-200 rounded-xl p-5 shadow-sm space-y-1">
                      <span className="text-[10px] text-gray-400 font-bold uppercase tracking-wider block">Total Mobilized</span>
                      <span className="text-2xl font-serif font-extrabold text-navy-900">{systemStats.totalPlaced}</span>
                    </div>
                    <div className="bg-white border border-gray-200 rounded-xl p-5 shadow-sm space-y-1">
                      <span className="text-[10px] text-gray-400 font-bold uppercase tracking-wider block">Gross Revenue</span>
                      <span className="text-2xl font-serif font-extrabold text-gold-600">{systemStats.annualRevenue}</span>
                    </div>
                  </div>
                  ...
                  <div className="grid grid-cols-1 md:grid-cols-2 gap-6 text-xs font-semibold text-gray-500">
                    <div className="bg-white border border-gray-200 rounded-xl p-6 shadow-sm space-y-4">
                      <h4 className="font-bold text-navy-900">Deployments by Target Destination</h4>
                      <div className="space-y-3 font-bold text-[11px] text-navy-900">
                        <div className="flex justify-between"><span>Saudi Arabia (KSA)</span><span className="text-gold-650">65% (806)</span></div>
                        <div className="flex justify-between"><span>United Arab Emirates (UAE)</span><span className="text-gold-650">20% (248)</span></div>
                        <div className="flex justify-between"><span>State of Qatar</span><span className="text-gold-650">10% (124)</span></div>
                        <div className="flex justify-between"><span>Schengen Zone (Europe)</span><span className="text-gold-650">5% (62)</span></div>
                      </div>
                    </div>

                    <div className="bg-white border border-gray-200 rounded-xl p-6 shadow-sm space-y-4">
                      <h4 className="font-bold text-navy-900">Active Pipeline Ratios</h4>
                      <div className="space-y-3 font-bold text-[11px] text-navy-900">
                        <div className="flex justify-between"><span>Biometrics Complete</span><span className="text-gold-650">45 Cases</span></div>
                        <div className="flex justify-between"><span>Embassy Queue Stamping</span><span className="text-gold-650">30 Cases</span></div>
                        <div className="flex justify-between"><span>Flight departures queue</span><span className="text-gold-650">15 Cases</span></div>
                        <div className="flex justify-between"><span>Medical approvals pending</span><span className="text-gold-650">25 Cases</span></div>
                      </div>
                    </div>
                  </div>
                </div>
                      ...
                      <tbody className="divide-y divide-gray-100 font-medium">
                        {employers.map((emp) => (
                          <tr key={emp.id} className="text-navy-900">
                            ...
                            <td className="py-4">
                              <span className={`px-2 py-0.5 rounded text-[9px] uppercase font-bold border ${emp.isVerified ? "bg-green-50 text-green-700 border-green-200" : "bg-amber-50 text-amber-700 border-amber-200 animate-pulse"}`}>
                            ...
                            <td className="py-4 text-right">
                              <button
                                ...
                                className={`text-[10px] font-bold uppercase tracking-wider px-3 py-1.5 rounded transition-colors cursor-pointer border ${emp.isVerified ? "border-red-200 text-red-650 bg-red-50/10 hover:bg-red-50" : "border-gold-500/30 text-gold-600 bg-gold-500/5 hover:bg-gold-500/10"}`}
                              >
                      ...
                      <tbody className="divide-y divide-gray-100 font-medium">
                        {candidates.map((cand) => (
                          <tr key={cand.id} className="text-navy-900">
                            ...
                            <td className="py-4 text-gray-500 truncate max-w-[150px]">{cand.skills || "Not Provided"}</td>
                            ...
                            <td className="py-4 text-right">
                              {cand.resumeUrl ? (
                                <a
                                  ...
                                  target="_blank"
                                  rel="noreferrer"
                                  className="text-gold-600 hover:underline font-bold inline-flex items-center space-x-1"
                                >
                      ...
                      <tbody className="divide-y divide-gray-100 font-medium">
                        {jobs.map((job) => (
                          <tr key={job.id} className="text-navy-900">
                            ...
                            <td className="py-4">
                              <span className={`px-2 py-0.5 rounded text-[9px] uppercase font-bold border ${job.status === "OPEN" ? "bg-green-50 text-green-700 border-green-200" : "bg-red-50 text-red-750 border-red-200"}`}>
                                {job.status}
                              </span>
                            </td>
                            <td className="py-4 text-right">
                              <button
                                ...
                                className={`text-[10px] font-bold uppercase tracking-wider px-3 py-1.5 rounded transition-colors cursor-pointer border ${job.status === "OPEN" ? "border-red-200 text-red-650 bg-red-50/10 hover:bg-red-50" : "border-green-200 text-green-650 bg-green-50/10 hover:bg-green-50"}`}
                              >
              ...
              {activeTab === "ats" && (
                <div className="space-y-4">
                  <div className="pb-2 border-b border-gray-200">
                    ...
                    <p className="text-xs text-gray-500 mt-0.5 font-sans">Full administrative drag/drop controls across all company recruitment campaigns.</p>
                  </div>
              ...
              {activeTab === "crm" && (
                <div className="bg-white border border-gray-200 rounded-xl p-6 shadow-sm space-y-6">
                  <div className="flex justify-between items-center pb-3 border-b border-gray-150">
                    <h3 className="font-serif text-sm font-bold text-navy-900 uppercase tracking-wider">CRM Inquiries Inbox</h3>
                    
                    <select
                      value={crmFilter}
                      onChange={(e) => setCrmFilter(e.target.value)}
                      className="bg-luxury-light border border-gray-200 text-navy-900 rounded p-1 text-[11px] font-semibold cursor-pointer"
                    >
                      ...
                      .map((inq) => (
                        <div
                          ...
                          className={`border rounded-xl p-5 space-y-3 text-xs ${
                            inq.status === "PENDING" ? "bg-amber-50/20 border-amber-200" : "bg-gray-50 border-gray-200"
                          }`}
                        >
                          <div className="flex justify-between items-start">
                            ...
                            <select
                              ...
                              className="bg-white border border-gray-200 text-navy-900 rounded p-1 text-[10px] font-bold outline-none cursor-pointer"
                            >
                              <option value="PENDING">PENDING</option>
                              <option value="CONTACTED">CONTACTED</option>
                              <option value="ARCHIVED">ARCHIVED</option>
                          ...
                          <p className="text-gray-600 leading-relaxed font-light bg-white p-3 rounded-lg border border-gray-150">
                            "{inq.message}"
                          </p>

                          <div className="flex flex-wrap gap-4 text-gray-400 font-semibold text-[9px] uppercase tracking-wider">
              ...
              {activeTab === "cms" && (
                <div className="grid grid-cols-1 lg:grid-cols-3 gap-8">
                  <div className="lg:col-span-2 bg-white border border-gray-200 rounded-xl p-6 shadow-sm space-y-4">
                    ...
                    <form onSubmit={handleAddBlog} className="space-y-3.5 text-xs text-gray-500 font-semibold">
                      <div className="grid grid-cols-1 sm:grid-cols-2 gap-4">
                        <div>
                          ...
                          <input
                            ...
                            className="w-full bg-luxury-light border border-gray-200 text-navy-900 rounded-lg p-2.5 outline-none font-medium"
                          />
                        </div>
                        <div>
                          ...
                          <select
                            ...
                            className="w-full bg-luxury-light border border-gray-200 text-navy-900 rounded-lg p-2.5 outline-none font-medium cursor-pointer"
                          >
                      ...
                      <div>
                        ...
                        <textarea
                          ...
                          className="w-full bg-luxury-light border border-gray-200 text-navy-900 rounded-lg p-2.5 outline-none font-medium resize-none"
                        />
                      </div>

                      <button
                        type="submit"
                        className="w-full bg-navy-900 hover:bg-navy-800 text-gold-500 font-bold uppercase tracking-wider py-3 rounded-lg border border-gold-500/10 cursor-pointer text-[10px]"
                      >
                        Publish Article
                      </button>
                    </form>
                  ...
                  <div className="lg:col-span-1 bg-white border border-gray-200 rounded-xl p-6 shadow-sm space-y-4">
                    <h3 className="font-serif font-bold text-xs text-navy-900 pb-2 border-b border-gray-150 uppercase tracking-wider flex items-center space-x-1.5">
                      <ImageIcon className="h-4.5 w-4.5 text-gold-500" />
                    ...
                    <div className="border-2 border-dashed border-gray-200 rounded-2xl p-10 text-center hover:border-gold-500 transition-colors">
                      <ImageIcon className="h-8 w-8 text-gold-500 mx-auto mb-2" />
                      <button
                        onClick={() => alert("Photo upload logged in public gallery database.")}
                        className="bg-navy-900 hover:bg-navy-800 text-gold-500 text-[10px] font-bold uppercase tracking-wider px-4 py-2.5 rounded-lg border border-gold-500/10 cursor-pointer"
                      >
                        Select Image
                      </button>
                    </div>
                ...
                <div className="bg-white border border-gray-200 rounded-xl p-6 shadow-sm space-y-6">
                  ...
                  <div className="space-y-4 text-xs font-semibold text-gray-500">
                    {[
                      ...
                      <div key={i} className="border border-gray-150 rounded-xl p-4 bg-luxury-light space-y-2">
                        <div className="flex justify-between items-center">
                          <span className="text-[9px] uppercase tracking-wider font-bold text-gold-600 bg-gold-500/10 px-2.5 py-0.5 rounded-lg">
                            {temp.channel}
                          </span>
                          <h4 className="font-bold text-navy-900 text-xs">{temp.name}</h4>
                        </div>
                        <p className="text-[11px] text-gray-400 font-light italic bg-white p-3 rounded-lg border border-gray-100">
                          "{temp.preview}"
                        </p>
                      </div>
                    ))}
              ...
              {activeTab === "system" && (
                <div className="grid grid-cols-1 lg:grid-cols-2 gap-8">
                  {/* Permissions */}
                  <div className="bg-white border border-gray-200 rounded-xl p-6 shadow-sm space-y-4">
                    <h3 className="font-serif font-bold text-xs text-navy-900 pb-2 border-b border-gray-150 uppercase tracking-wider">Role Permissions</h3>
                    <div className="space-y-3 text-xs font-semibold text-navy-900">
                      {[
                        ...
                        <div key={i} className="border border-gray-200 p-3 rounded bg-luxury-light flex justify-between items-center">
                          <div>
                            <h4 className="font-bold">{perm.role}</h4>
                            <p className="text-[10px] text-gray-400 font-light mt-0.5">{perm.desc}</p>
                          </div>
                          <Lock className="h-4 w-4 text-gold-500 shrink-0" />
                        </div>
                      ))}
                    </div>
                  </div>
                  ...
                  <div className="bg-white border border-gray-200 rounded-xl p-6 shadow-sm space-y-6 font-sans">
                    <div className="space-y-4">
                      ...
                      {seoMsg && (
                        <div className="p-3 bg-green-50 border border-green-200 text-green-700 rounded-lg text-[11px] font-semibold">
                          {seoMsg}
                        </div>
                      )}
                      <form onSubmit={handleSaveSEO} className="space-y-3.5 text-xs text-gray-500 font-semibold">
                        <div>
                          <label className="block mb-0.5 text-navy-900">Page Meta Title *</label>
                          <input
                            type="text"
                            required
                            value={seoTitle}
                        ...
                        <div>
                          <label className="block mb-0.5 text-navy-900">SEO Keywords *</label>
                          <input
                            type="text"
                            required
                            value={seoKeywords}
                        ...
                        <button type="submit" className="w-full bg-navy-900 hover:bg-navy-800 text-gold-500 font-bold uppercase tracking-wider py-2.5 rounded-lg border border-gold-500/10 cursor-pointer text-[9px]">
                          Save SEO Settings
                        </button>
                      </form>
                    </div>
                    ...
                    <div className="space-y-3 pt-4 border-t border-gray-100">
                      ...
                      <button
                        ...
                        className="w-full bg-gold-500 hover:bg-gold-400 text-navy-950 font-bold uppercase tracking-wider py-3 rounded-lg text-[10px] shadow flex items-center justify-center space-x-1.5 cursor-pointer border border-gold-500/10 font-sans"
                      >
